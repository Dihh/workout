...
export default {
    ...
    data() {
        return {
            dayWorkout: null,
            lastExecutedWeight: null,
            maxExecutedWeight: null,
            id: '',
            loading: true,
    ...
    methods: {
        ...
        async buildCharts() {
            const records = await this.dayWorkoutsController.select_by_exercise(this.dayWorkout.exercise_id)

            const executed = records.filter(r => r.executed && r.executed != '0')
            const weights = executed.map(r => Number(r.weight))
            this.maxExecutedWeight = weights.length ? Math.max(...weights) : null
            const lastRecord = executed.sort((a, b) => b.date.localeCompare(a.date))[0]
            this.lastExecutedWeight = lastRecord ? lastRecord.weight : null
        ...
        renderWeightChart(labels, data) {
            const { canvas, gradient } = this.makeGradient('dw-chart-weight', 'rgba(100,25,135,0.35)', 'rgba(100,25,135,0)')
            ...
            this._chartWeight = new Chart(canvas, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: { backgroundColor: '#641987', callbacks: { label: ctx => ` ${ctx.parsed.y} kg` } }
                    },
                    scales: {
        ...
        renderExecChart(labels, data) {
            const { canvas, gradient } = this.makeGradient('dw-chart-exec', 'rgba(100,25,135,0.35)', 'rgba(100,25,135,0)')
            ...
            this._chartExec = new Chart(canvas, {
                ...
                options: {
                    ...
                    plugins: {
                        legend: { display: false },
                        tooltip: { backgroundColor: '#641987', callbacks: { label: ctx => ` ${ctx.parsed.y} execuções` } }
                    },
                    scales: {
